...
RM1 = generate_random_matrix(len(VOC), len_seq * len(VOC))
CMM = create_CMM_layer1(RM1)

predicted_text = "La tecnologia ha trasformato"

PM = generate_random_matrix(1, len_seq*len(VOC))
...
def layer1():
    ...
    for i in range(4, len(words)):
        vector = words[i - 4:i]
        sequence = seq(vector)
        encoded_sequence = np.matmul(sequence, np.transpose(PM))

        # Converti l'array in una lista di valori
        encoded_sequence_list = encoded_sequence.flatten().tolist()

        # Estrai il primo valore della lista
        first_value = encoded_sequence_list[0]
        encoded_text.append(first_value)

    return encoded_text

def create_CMM_layer2(RM2):
    temp = np.zeros((len(encoded_text),len(encoded_text)))

    for s in range(len(encoded_text) - 4):
        sequence = encoded_text[s:s + 4]
        target = encoded_text[s + 4]

        v1 = np.transpose(seq(sequence))

        v1_reduced = np.matmul(RM2, v1)
...
encoded_text = layer1()
RM2 = generate_random_matrix(len(VOC), len_seq * len(VOC))
